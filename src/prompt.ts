...
export function buildWorkerSystem(): string {
  return [
    ...
    '4. 进度以材料末尾为准：前面提出、后面已关闭的问题必须写入「已完成」，禁止留在「当前状态与开放问题」。开放问题只列到材料结束时仍未关闭的事项。材料里带「压缩」标记的仍是完整时间线上的一轮（只压中间、保留开头与结尾），必须按时间顺序读完，不得当成「没发生过」。',
    '',
    ...
    '写作要求：',
    '- 每段 1~6 条要点，宁缺毋滥；',
    '- 「已完成」逐条带文件路径与验证结果；已关闭的原开放问题也写在这里，并注明关闭依据；',
