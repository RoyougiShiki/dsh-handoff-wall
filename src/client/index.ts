/**
 * @dsh-external/dsh-handoff-board — client 面板（conversation.view slot）。
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.slots.inject('conversation.view', () =>
    ctx.slots.register({
      ...
      id: '@dsh-external/dsh-handoff-board-panel',
      label: () => "@dsh-external/dsh-handoff-board",
      component: () => ({
        render() {
          const el = document.createElement('div')
          el.textContent = "@dsh-external/dsh-handoff-board" + ' 面板（host API: /@dsh-external/dsh-handoff-board/api）'
  ...
  ), '@dsh-external/dsh-handoff-board: panel')
}
